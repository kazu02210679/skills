...
4. **Collect local evidence.** Use `run-verification --argv` only with a closed `python -m unittest` spec: exact argv, explicit `TEST-*`→path mapping, canonical artifacts, and one executed test per declared path. It runs shell-free, re-hashes paths before/after, rejects nonzero/empty/short output, and records G3 outputs.
