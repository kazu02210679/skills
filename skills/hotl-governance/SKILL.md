...
4. **Collect local evidence.** Use `run-verification --argv` only with one policy-frozen canonical argv. It executes without a shell, rejects nonzero status, and records exact outputs and the controller-owned receipt for G3.
