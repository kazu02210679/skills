...
```powershell
python skills/gpt-pro-codex-loop/scripts/gpc_loop.py init --repo REPOSITORY --task TASK --request REQUEST.md --repository-context CONTEXT.md --model-policy PRO_CLASS --review-policy FINAL_ONLY --approved-existing-path-manifest ..\REPOSITORY-TASK-approved-existing-paths.json [--governance-context HOTL-CONTEXT.json]
```
...
Attestation schema v4 makes `GPT-5.6 Sol` the sole `PRO_CLASS` model. Model identity and reasoning strength are separate UI fields; v4 accepts only the exact `GPT-5.6 Sol` + `Pro` + Pro-capable-plan tuple. A coherent unbound v3 or v2 state, or fieldless legacy state, with null URL/model/reasoning/plan upgrades to null-v4 on its next transition. Every bound, partial, malformed, noncanonical, boolean/float-version, or otherwise unsupported pre-v4 attestation returns read-only `LEGACY_STATE_RESTART_REQUIRED` with no command; every receipt export for requirements, review, and final uses the same classification. Preserve the run and start a new task slug. Never invent model identity.
