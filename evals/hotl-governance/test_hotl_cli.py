...
class HotlCliTests(unittest.TestCase):
    ...
    def _export_frozen_gpt_requirements(
        ...
        requirements = {
            ...
            "risk_items": [{"required_mitigation": "Require AC-1 evidence.", "id": "RISK-1", "risk": "Evidence may be incomplete."}],
            "schema_version": 1,
            "scope_changed": False,
        ...
        self.authority_snapshot_digest = controller.gpt_governance_authority_digest(binding)
        self.receipt_nonce = controller.gpt_governance_nonce(binding)
        self.gpt_binding = binding
        hotl_requirements = {
            "requirements": ["REQ-1"], "source_artifact": requirements,
            "source_digest": contract.canonical_digest(requirements),
        }
        hotl_policy = {
        ...
        self.assertEqual("REQUIREMENTS", initialized["state"])
        context_path = self._write("gpt-hotl-context.json", controller.export_governance_context(self.repository, self.execution))
        ...
        gpt_controller.initialize_run(
            ...
            "PRO_CLASS",
            None,
            governance_context_path=context_path,
        )
    ...
    def test_malformed_unhashable_policy_and_review_values_return_stable_envelopes(self) -> None:
        ...
        self.assertFalse(result["ok"])
        self.assertEqual("INVALID_RECEIPT_IDENTITY", result["error"]["code"])
    ...
    def test_replayed_receipt_is_rejected(self) -> None:
        receipt, artifact, receipt_bytes = self._export_frozen_gpt_requirements()
        self._init_from_gpt_requirements((receipt, artifact, receipt_bytes))
    ...
    def test_gpt_requirements_receipt_keeps_frozen_null_admission_bindings(self) -> None:
        requirements, artifact, receipt_bytes = self._export_frozen_gpt_requirements()
        self.assertIsNone(requirements["evidence_set_digest"])
        self.assertIsNone(requirements["cycle_id"])
        self._init_from_gpt_requirements((requirements, artifact, receipt_bytes))
        frozen_record = self._projection()["receipt_records"][requirements["receipt_id"]]
    ...
    def test_g1_rejects_a_canonical_but_unissued_gpt_requirements_receipt(self) -> None:
        """Removing source-identity replay would admit an arbitrary receipt ID."""
        self._init()
        forged = self._receipt("requirements", "RCP-ARBITRARY-GPT")
        forged["transaction_id"] = "gpc-loop-hotl-fixture:requirements-01"
        result = self._import(forged, "arbitrary-gpt-requirements.json")
        self.assertFalse(result["ok"], result)
    ...
    def test_g1_rejects_a_gpt_receipt_without_current_hotl_context(self) -> None:
        ...
        self._init()
        receipt = self._receipt("requirements", "RCP-NO-HOTL-CONTEXT")
        receipt["transaction_id"] = "gpc-loop-hotl-fixture:requirements-01"
        identity = {
            field: receipt[field]
            for field in (
        ...
        receipt["receipt_id"] = "RCP-GPC-" + hashlib.sha256(
            contract.canonical_json_bytes(identity)
        ).hexdigest()[:20].upper()
        result = self._import(receipt, "missing-hotl-context.json")
        self.assertFalse(result["ok"], result)
        self.assertEqual("HOTL_CONTEXT_MISMATCH", result["error"]["code"])
    ...
    def test_gpt_review_and_final_receipts_receive_current_live_admission_bindings(self) -> None:
        ...
        review, review_bytes, final, final_bytes = authoritative
        self.assertIsNone(review["evidence_set_digest"])
        self.assertIsNone(review["cycle_id"])
        self._import_receipt_bytes(review_bytes, "gpt-rev-live.json")
        self._import_receipt_bytes(final_bytes, "gpt-final-live.json")
        records = self._projection()["receipt_records"]
        for receipt_id in (review["receipt_id"], final["receipt_id"]):
    ...
    def test_privileged_receipt_validation_and_append_share_one_run_lock(self) -> None:
        receipt, _, _ = self._export_frozen_gpt_requirements()
        paths = store.resolve_run(self.repository, self.execution)
    ...
    def test_import_sol_receipt_rejects_a_rehashed_invalid_timestamp(self) -> None:
        ...
        self._init()
        scenario = {"authority_snapshot_digest": AUTHORITY, "execution_id": self.execution, "input_digest": SNAPSHOT, "intent": "gpt-pro-only", "invocation_id": "INV-BAD-TIME", "nonce": NONCE, "output_digest": SNAPSHOT}
        receipt = sol_receipts.governance_receipt(scenario, sol_receipts.route(scenario), None)
        receipt["issued_at_unix"] = "forged"
        identity = dict(receipt)
        identity.pop("receipt_id")
        receipt["receipt_id"] = "RCP-SOL-" + hashlib.sha256(json.dumps(identity, ensure_ascii=False, separators=(",", ":"), sort_keys=True).encode("utf-8")).hexdigest()[:20].upper()
        path = self.inputs / "forged-sol-time.json"
        path.write_bytes(json.dumps(receipt, ensure_ascii=False, separators=(",", ":"), sort_keys=True).encode("utf-8"))
        result = cli.main_json(["import-sol-receipt", "--repo", str(self.repository), "--execution", self.execution, "--receipt", str(path)])
        self.assertFalse(result["ok"], result)

    def test_import_sol_receipt_rejects_a_rehashed_incomplete_consultation_binding(self) -> None:
        """Task 7 validation must close every runtime binding field, not just its ID."""
        self._init()
        scenario = {
            "authority_snapshot_digest": AUTHORITY,
            "execution_id": self.execution,
        ...
        receipt = sol_receipts.governance_receipt(
            scenario, sol_receipts.route(scenario), {"disposition": "accept"}
        )
        self.assertEqual("consultation", receipt["receipt_type"])
        receipt["binding"] = dict(receipt["binding"], model="")
        identity = dict(receipt)
        identity.pop("receipt_id")
        receipt["receipt_id"] = "RCP-SOL-" + hashlib.sha256(
            json.dumps(
        ...
        with self.assertRaises(sol_receipts.ReceiptError):
            sol_receipts.validate_governance_receipt(raw)
        result = cli.main_json(
            [
                "import-sol-receipt",
                "--repo", str(self.repository),
                "--execution", self.execution,
                "--receipt", str(self._write("forged-sol-binding.json", receipt)),
            ]
        )
        self.assertFalse(result["ok"], result)
        self.assertEqual("INVALID_SOL_RECEIPT", result["error"]["code"])

    def test_status_advertises_only_controller_owned_gate_commands(self) -> None:
        """Generic record/approve must not be advertised for G2/G3/Sol ingress."""
        self.assertEqual(["record-implementation"], cli._next_commands({"state": "IMPLEMENT", "allowed_transitions": []}))
        self.assertEqual(["run-verification"], cli._next_commands({"state": "LOCAL_VERIFY", "allowed_transitions": []}))
        self.assertEqual(["import-receipt", "import-sol-receipt"], cli._next_commands({"state": "SEMANTIC_REVIEW", "allowed_transitions": []}))
